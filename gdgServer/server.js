...
app.get('/hello/:name/:age', function(httpRequest, httpResponse) {
    var name = httpRequest.params.name;
    var age = httpRequest.params.age;
    httpResponse.send('Hello, ' + name + ". You're " + age + ' years old!');
});
...
// var url = `http://api.steampowered.com/ISteamUserStats/GetSchemaForGame/v2/?key=${steamKey}&appid=8930`;
...
// `http://api.steampowered.com/ISteamUserStats/GetSchemaForGame/v2/?key=${steamKey}&appid=8930`
// ```js
...
app.get('/steam/game/:appid/achievements', function(httpRequest, httpResponse) {
    // Calculate the Steam API URL we want to use
    var url = `http://api.steampowered.com/ISteamUserStats/GetSchemaForGame/v2/?key=${steamKey}&appid=` +
